...
//        }
    ...
    public Image hanldeToImage(MultipartFile file, String directory) {
        boolean isStoredFailed = false;
        log.info("imagesService:hanldeToImage execution started.");
        ...
        if (file != null && file.getContentType() != null && !file.isEmpty() && file.getOriginalFilename() != null) {
            ...
            try {
                ...
                log.error("Exception occurred while persisting imagesService:hanldeToImag config file , Exception message {}", ex.getMessage());
                isStoredFailed = true;
            }
        }
        if (!isStoredFailed) {
